...
public class TokenBase<T extends TokenBase> {
    ...
    public String getCommentContents() throws InvalidMarkupException{
    	...
    	if (!markup.startsWith("<!--") || !markup.endsWith("-->")) throw new InvalidMarkupException("Failed to parse comment",this);
    ...
    protected void parseTagFromMatcher(Matcher m) throws InvalidMarkupException{
        ...
        //Parse attributes
        String attrText = m.group(3); if (attrText == null) attrText = "";
        Matcher ma = attributePair.matcher(attrText);
        int index = 0;
        while (ma.find(index)){
            if (attrs == null) attrs =  new TreeMap<String,String>(String.CASE_INSENSITIVE_ORDER);//Default Collator case-insensitive (TETERARY) //FIixed bug #80 on Feb 2.
    ...
    public T appendToAttributeSmart(String attributeName, String value) throws InvalidMarkupException{
        ...
        if (attrs.containsKey(attributeName)){
            ...
            String suffix = "";
            while (value.endsWith(delimiter)) { suffix += delimiter; value = value.substring(0, value.length() -1);}

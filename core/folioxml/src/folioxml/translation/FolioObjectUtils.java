...
public class FolioObjectUtils{
    ...
    public static SlxToken translateObject(FolioToken ft) throws InvalidMarkupException{
    	 ...
    	 */
    	if (ft.isClosing()) throw new InvalidMarkupException("Object (OB) tags are all self-closing. Closing </OB> tags should not exist",ft);
    	...
    	while(ix < opts.size()){
    		...
    		if (FolioCssUtils.isNumber(s)){
    			...
    			if (!hasWidth) {
    				...
    				hasHeight = true;
    			} else if (hasHeight) throw new InvalidMarkupException("Only Width and Height may be specified on OB tags. Unexpected number");
    			i++;
    		}
    ...
    public static SlxToken translateObjectDefinition(FolioToken ft) throws InvalidMarkupException{
    	...
    	if (ft.count() < 4) throw new InvalidMarkupException("Object Definition (OD) tags must have 4 or more arguments.",ft);

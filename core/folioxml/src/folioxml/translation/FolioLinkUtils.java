...
public class FolioLinkUtils {
    ...
    public static SlxToken translate(FolioToken t) throws InvalidMarkupException {
        ...
        if (isClosingLinkTag(t)) return new SlxToken("</link>");
            ...
            if (t.count() < 2 && !t.matches("UL"))
                throw new InvalidMarkupException("All links (PL, WW, QL, JL, PX, OL, DL, ML) except User Link (UL) must provide at least 2 arguments.", t);
            ...
            if (t.matches("WW|PL")) {
                ...
                if (t.count() > 2)
                    throw new InvalidMarkupException("WW, PL, and DL links can only have 2 arguments.", t);
                ...
                if (t.count() > 2)
                    throw new InvalidMarkupException("PX (Named popup links) can only have 2 arguments.", t);
                ...
                if (t.count() > 3)
                    throw new InvalidMarkupException("JL (Jump link) may only have 3 arguments." + t.text);
    ...
    public static String translateToFolio(SlxToken t, CssClassCleaner css) throws InvalidMarkupException {
    	...
    	*/
        ...
        if (t.get("jumpDestination") != null) {
            if (t.get("infobase") != null)
                return "<JL:\"" + style + "\",\"" + t.get("jumpdestination") + "\",\"" + t.get("infobase") + "\">";

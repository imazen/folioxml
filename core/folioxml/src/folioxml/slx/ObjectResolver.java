...
public class ObjectResolver {
	...
	public void fixToken(SlxToken t) throws InvalidMarkupException, IOException{
		...
		if (t.matches("link")){
			...
			if (key == null || t.get("infobase") != null) return; //Not the right type of link. Only local infobase links are supported.
		}else {
			key = t.get("name");
		}
		...
		if (newAttrs == null) throw new InvalidMarkupException("Object named \"" + t.get("name") + "\" was not found in the definiton.",t);
		
		//TODO: Isn't type a required attribute?
		
		if (t.matches("object") && newAttrs.get("type") != null && newAttrs.get("type").equalsIgnoreCase("data-link")) throw new InvalidMarkupException("Data-link objects cannot be embedded, only linked to.");

...
public class SlxRecord extends SlxToken implements ISlxTokenWriter {
    ...
    public void setUnderlyingWriter(ISlxTokenWriter base) {
        if (base != this)
            throw new UnsupportedOperationException("The only supported underlying receiver for an SlxRecord is itself. It must be the end point in the filter chain.");
    }
    ...
    public void calculateParent(SlxRecord lastLevelRecord, boolean strict) throws InvalidMarkupException {
        ...
        if (lastLevelRecord == null) {
            if (strict && !levelType.equalsIgnoreCase("root"))
                throw new InvalidMarkupException("The first record must have level=\"root\".", this);
            return;
        }
        //Find root node
        SlxRecord root = getRoot(lastLevelRecord);
        if (strict && (root == null || !"root".equalsIgnoreCase(root.get("level"))))
            throw new InvalidMarkupException("Root node not found. Root node must have level=\"root\" and be the top of the heirarchy.", lastLevelRecord);
        ...
        for (SlxToken t : this.tokens) {
            ...
            if (!stack.has("note|popup|table")) {
                if (t.isTextOrEntity()) {
                    ...
                    if (firstParagraph)
                        all.append(s); //The failover is the entire first paragraph (that contains text or whitespace)
    ...
    private List<String> getCurrentLevels(SlxRecord current) throws InvalidMarkupException {
        ...
        while (level != null) {
            ...
            if (l == null)
                throw new InvalidMarkupException("Non-level record cannot be the parent of another record", level);

...
public class SlxValidator{
    ...
    */
    ...
    public void preValidate(SlxToken t) throws InvalidMarkupException{
        ...
        if (t.isContent() && isInRoot) throw new InvalidMarkupException("Text and entities are not allowed at the top of the file. They cannot appear in the root record.",t);
        ...
        if (t.isTag() && (t.isOpening() || t.isGhost)){
            if (topContext == null && !t.matches("record")) throw new InvalidMarkupException("Only record tags are allowed at the root level. All tags must be contained within a record.", t);
            
            
            
            if (isInRoot){
                if (!t.matches("infobase-meta|style-def|object-def|namedPopup")) throw new InvalidMarkupException("Only style definitions, named popups, and infobase information can appear in the root record",t);
                    
            }else{
                if (t.matches("style-def|infobase-meta|object-def")) throw new InvalidMarkupException("infobase-meta, object-def, and style-def are only allowed in the root record, at the top of the file.",t);
    ...
    public void validate(SlxToken t) throws InvalidMarkupException{
        ...
        if (t.isTag()){
            ...
            if (t.isOpening()){
                ...
                if (t.matches("link") && stack.has("link")) throw new InvalidMarkupException("Links cannot be nested. " + t+ " was found inside " + stack.get("link"),t);
	...
	public static boolean isAllowedInside(SlxToken t, SlxToken parent) throws InvalidMarkupException {
		...
		if (parent.matches("span|p|td|th|infobase-meta|note|popup|namedPopup|link")) return true; //Cannot be directly inside - table|tr|object|br|bookmark|style-def|object-def
